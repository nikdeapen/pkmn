...
impl Client {
    ...
    ///
    /// Scraped ids are mapped to our local ids via the set-id records so renamed and
    /// combined sets are not reported as unknown.
    pub fn scrape_unknown_sets(&self) -> Result<Vec<CardSet>, web_scrape::Error> {
        ...
        let scraped: Vec<RawSet> = self.scrape_sets()?;
        let cleaned: Vec<CardSet> = self.clean_sets(&scraped).map_err(ScrapeError::Other)?;
        Ok(cleaned
            .into_iter()
            .filter(|set| {
